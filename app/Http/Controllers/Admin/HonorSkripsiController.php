...
use App\Models\HonorSkripsiTugas;
...
class HonorSkripsiController extends Controller
{
    ...
    {
        $honor_skripsi = HonorSkripsi::with(['semester', 'tugas'])->get();

        return Inertia::render('Admin/Bendahara/honor-skripsi/index', [
            'honor_skripsi' => $honor_skripsi,
        ]);
    }
    ...
    //         return redirect()->back()->with('success', 'Honor skripsi berhasil ditambahkan.');
    //     } catch (\Exception $e) {
    ...
    public function update(Request $request, HonorSkripsi $honorSkripsi)
    {
        try {
            $validated = $request->validate([
                'semester_id' => 'required|exists:semesters,id',
    ...
    {
        try {
            $honor = HonorSkripsi::findOrFail($id);
    ...
    public function invoicePdf($id)
    {
        ...
        $honor = HonorSkripsi::with(['semester', 'tugas'])->findOrFail($id);

        // Load view dan generate PDF
        $pdf = Pdf::loadView('inv-honor-skripsi', compact('honor'))
            ->setPaper('a5', 'portrait');

        // Stream file PDF ke browser
        return $pdf->stream('Invoice_Honorarium_Skripsi_' . str_replace(' ', '_', $honor->nama) . '.pdf');
    }
    ...
    public function laporanPdf(Request $request)
    {
        ...
        $honorList = HonorSkripsi::with(['semester', 'tugas'])
        ...
        $pdf = Pdf::loadView('laporan-honor-skripsi', [
            'honorList' => $honorList,
            'semester' => $semester,
        ]);

        return $pdf->stream('Laporan_Honor_Skripsi_Semester_' . $semester->nama . '.pdf');
    }
